...
public class BST<Key extends Comparable<Key>, Value> {
    ...
    private Value get(Node x, Key key) {
        ...
        // L?nh if
    if (cmp < 0) {
        // N?u key c?n tìm n?m ? bên trái c?a node hi?n t?i
        // ti?p t?c tìm ki?m trên cây con trái c?a node hi?n t?i
        return get(x.left, key);
    } else if (cmp > 0) {
        // N?u key c?n tìm n?m ? bên ph?i c?a node hi?n t?i
        // ti?p t?c tìm ki?m trên cây con ph?i c?a node hi?n t?i
        return get(x.right, key);
    } else {
        // N?u key c?n tìm b?ng v?i key c?a node hi?n t?i
        // tr? v? giá tr? t??ng ?ng v?i key ?ó
        return x.val;
    }
}
