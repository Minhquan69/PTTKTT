...
public class FordFulkerson {
    ...
    private static Bag<FlowEdge> bag;
    ...
    public FordFulkerson(FlowNetwork G, int s, int t) {
        ...
        value = excess(G, t); // tinh gia tri luong vao t, xuat phat bang 0
    ...
    private boolean check(FlowNetwork G, int s, int t) {
        ...
        bag = new Bag<FlowEdge>(); // t?o m?t Bag ?? l?u các c?nh trong c?t nh? nh?t
    double mincutValue = 0.0;
    for (int v = 0; v < G.V(); v++) {
        for (FlowEdge e : G.adj(v)) {
            if ((v == e.from()) && inCut(e.from()) && !inCut(e.to())) {
                bag.add(e); // thêm c?nh e vào Bag
                mincutValue += e.capacity();
            }
        }
    }

    // in ra các c?nh trong c?t nh? nh?t
    StdOut.print("Edges in min cut: ");
    for (FlowEdge e : bag) {
        StdOut.print(e + " ");
    }
    StdOut.println();
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Flow of Min cut = " + mincutValue);

        StdOut.println();
        // Gia tri luong cuc dai
