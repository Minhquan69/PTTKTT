 ...
 *  @author Kevin Wayne
 */
public class FordFulkerson_cut {
    private static final double FLOATING_POINT_EPSILON = 1E-11;
 private static Queue<FlowEdge> minCutEdges = new Queue<FlowEdge>();
     ...
     */
    public FordFulkerson_cut(FlowNetwork G, int s, int t) {
        V = G.V();
        validate(s);
        validate(t);
...
private boolean check(FlowNetwork G, int s, int t) {
    minCutEdges = new Queue<FlowEdge>(); // Kh?i t?o Bag m?i trong m?i l?n g?i check()
    ...
    for (int v = 0; v < G.V(); v++) {
        for (FlowEdge e : G.adj(v)) {
            if ((v == e.from()) && inCut(e.from()) && !inCut(e.to())) {
                ...
                minCutEdges.enqueue(e); // Thêm c?nh vào Bag ? ?ây
            }
        }
    }
    ...
    public static void main(String[] args) {
        ...
        // print min-cut edges
   //    StdOut.println("Min cut edges:");
   //    for (FlowEdge e : minCutEdges) {
      //     StdOut.println(e);
      //  }
    }
