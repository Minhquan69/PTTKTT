...
public class FarthestPair {
     ...
     */
    public FarthestPair(Point2D[] points) throws IllegalArgumentException {
   ...
   /**
    ...
    public static void main(String[] args) throws FileNotFoundException {
        File file = new File("rs1423.txt"); // ???ng d?n t?i t?p ch?a d? li?u ?i?m
        Scanner scanner = new Scanner(file);
        int n = scanner.nextInt(); // ??c s? l??ng ?i?m

        Point2D[] points = new Point2D[n]; // T?o m?ng ?? l?u tr? ?i?m

        // ??c t?a ?? c?a các ?i?m t? t?p và t?o ??i t??ng Point2D t??ng ?ng
        for (int i = 0; i < n; i++) {
        ...
        FarthestPair farthest = new FarthestPair(points); // Tìm c?p ?i?m xa nh?t
        System.out.println(farthest.distance() + " tu " + farthest.either() + " den " + farthest.other());
