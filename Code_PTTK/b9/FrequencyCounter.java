...
public class FrequencyCounter {
    ...
    public static void main(String[] args)throws IOException {
        ...
        int distinct = 0, words = 0; //distinct : so tu khac nhau, words : nhung tu > nguong
        int minlen = Integer.parseInt(args[0]);//ng??ng
        ...
        while (!StdIn.isEmpty()) {
            ...
            words++;
            if (st.contains(key)) {
                st.put(key, st.get(key) + 1); //them vao cay tim kiem va tang value(lan xuat hien) len 1 
            }
            else {
                st.put(key, 1);
                distinct++;
        ...
        for (String word : st.keys()) {
            if (st.get(word) > st.get(max)) //lay ra word co value max
                max = word;
        }
